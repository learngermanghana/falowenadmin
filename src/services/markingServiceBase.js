...
export async function updateFirestoreScore(existingScore = {}, edits = {}) {
  const assignment = normalize(edits.assignment ?? existingScore.assignment);
  const assignmentId = normalize(edits.assignmentId ?? edits.assignment_id ?? existingScore.assignmentId ?? existingScore.assignment_id);
  const score = normalize(edits.score ?? existingScore.score ?? existingScore.finalScore);
  const studentCode = normalize(existingScore.studentCode || existingScore.studentcode);

  if (!studentCode) throw new Error("Cannot edit score: missing student code.");
  if (!assignment && !assignmentId) throw new Error("Cannot edit score: missing assignment.");
  if (score === "" || !Number.isFinite(Number(score))) throw new Error("Cannot edit score: score must be a number.");
  ...
  const updated = {
    ...
    name: normalize(edits.name ?? existingScore.name ?? existingScore.studentName),
    assignment,
    assignment_id: assignmentId,
    assignmentId,
    score: Number(score),
    finalScore: Number(score),
    comments: normalize(edits.comments ?? existingScore.comments ?? existingScore.feedback),
    feedback: normalize(edits.comments ?? existingScore.comments ?? existingScore.feedback),
    level: normalize(edits.level ?? existingScore.level),
    manuallyEdited: true,
    editedAt: now,
    updatedAt: now,
  };
  const dedupeId = existingScore.id || existingScore.dedupe_id || scoreDedupeId(updated);
  await setDoc(doc(db, "scores", dedupeId), { ...updated, dedupe_id: dedupeId }, { merge: true });
  return { ...updated, id: dedupeId, dedupe_id: dedupeId };
}
