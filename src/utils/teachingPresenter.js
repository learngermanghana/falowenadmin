...
function buildAdvancedGrammarItems(slide = {}, support = {}) { const source = Array.isArray(support.grammarFocusEn) ? support.grammarFocusEn : []; return [...new Set(source.flatMap(advancedGrammarItemsDe))]; }
...
function buildPresenterV2Stages(slide = {}, topicLabel = "") {
  ...
  const stages = [
    ...
    { id: "questions", type: "question-reveal", kicker: "Sprechen", title: advanced ? "Sprechtraining" : "Speaking questions", items: Array.isArray(slide.studentQuestionsDe) ? slide.studentQuestionsDe : [], supportItems: Array.isArray(slide.speakingModels) && slide.speakingModels.length ? slide.speakingModels.map((item) => item.modelAnswerDe).filter(Boolean) : (Array.isArray(support.modelExamplesDe) ? support.modelExamplesDe : []), questionModels: Array.isArray(slide.speakingModels) ? slide.speakingModels : [], requiresQuestionModel: ["A2", "B1"].includes(String(slide.course || "").toUpperCase()), suggestedMinutes: interactionMinutes(slide, 3) || 10 },
