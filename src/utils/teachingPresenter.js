...
function buildAdvancedGrammarItems(slide = {}, support = {}) { if (normalizedAssignmentId(slide) === "B2-1.1") return ["Adjektivendungen vor Nomen: ein ruhiger Mensch · eine prägende Erfahrung · einen zuverlässigen Menschen.", "Neue Kontrastmittel: während · hingegen · auf der einen Seite … auf der anderen Seite · im Gegensatz dazu."]; const source = Array.isArray(support.grammarFocusEn) ? support.grammarFocusEn : []; return [...new Set(source.flatMap(advancedGrammarItemsDe))]; }
function buildAdvancedMistakes(slide = {}) { if (normalizedAssignmentId(slide) === "B2-1.1") return ["Vor einem Nomen braucht das Adjektiv eine Endung: ein ruhiger Mensch.", "Im maskulinen Akkusativ: einen zuverlässigen Menschen.", "Heute nicht auf bekannte B1-Konnektoren ausweichen; gezielt während, hingegen und im Gegensatz dazu üben."]; if (classroomLevel(slide) === "C1") return ["Komplexe Strukturen nur verwenden, wenn Wortstellung und Bezug eindeutig bleiben.", "Abstrakte Aussagen immer mit Beispiel, Folge oder betroffener Gruppe konkretisieren.", "Ein Gegenargument nicht nur nennen, sondern anschließend darauf reagieren."]; return ["Nicht nur Ideen aufzählen: Aussage → Grund → Beispiel.", "Bei Nebensätzen auf die Verbendstellung achten.", "Nicht denselben Konnektor ständig wiederholen; die neue Zielstruktur bewusst variieren."]; }
function teacherNoteFromFlow(flow = [], index = 0, fallback = "") { return String(flow[index]?.detailEn || fallback).trim(); }

function buildB2Day1Practice(slide = {}, support = {}, flow = []) { return [
  { title: "1. Adjektivendungen", instruction: "Ergänze die richtige Form.", prompts: ["Ich bin ein ___ Mensch. (ruhig)", "Eine ___ Erfahrung hat mich geprägt. (wichtig)", "Ich beschreibe einen ___ Menschen. (zuverlässig)"], modelItems: ["ein ruhiger Mensch", "eine wichtige Erfahrung", "einen zuverlässigen Menschen"], teacherNote: "Check the endings quickly; do not reteach the full adjective-declension table.", minutes: 7 },
  { title: "2. Kontrast mit während", instruction: "Verbinde die beiden Aussagen mit während.", prompts: ["Im Beruf bin ich eher zurückhaltend. Mit Freunden spreche ich sehr offen."], modelItems: ["Während ich im Beruf eher zurückhaltend bin, spreche ich mit Freunden sehr offen."], teacherNote: "Use one clear contrast and check verb position before moving on.", minutes: 6 },
  { title: "3. Neue B2-Konnektoren", instruction: "Formuliere denselben Gegensatz zweimal neu.", prompts: ["Nutze einmal hingegen.", "Nutze einmal im Gegensatz dazu oder auf der einen Seite … auf der anderen Seite."], modelItems: ["Im Beruf bin ich eher zurückhaltend. Mit Freunden hingegen spreche ich sehr offen.", "Auf der einen Seite bin ich eher ruhig, auf der anderen Seite übernehme ich in wichtigen Situationen gern Verantwortung."], teacherNote: "Focus on the new B2 contrast expressions; skip deshalb, denn and weil drills here.", minutes: 8 },
  { title: "4. Sprechen", instruction: "Beschreibe zwei Seiten deiner Persönlichkeit und erkläre, wann sie sichtbar werden.", prompts: ["Sprich 60–90 Sekunden und nutze mindestens zwei neue Kontrastmittel sowie ein konkretes Beispiel."], modelItems: Array.isArray(support.modelExamplesDe) ? support.modelExamplesDe.slice(0, 2) : [], teacherNote: "Let the student finish the response, then correct the target connector and adjective-ending errors.", minutes: 10 },
]; }
...
function buildPresenterV2Stages(slide = {}, topicLabel = "") {
  ...
  const stages = [
    ...
    { id: "questions", type: "question-reveal", kicker: "Sprechen", title: advanced ? "Sprechtraining" : "Speaking questions", items: Array.isArray(slide.studentQuestionsDe) ? slide.studentQuestionsDe : [], supportItems: Array.isArray(support.modelExamplesDe) ? support.modelExamplesDe : [], questionModels: Array.isArray(slide.speakingModels) ? slide.speakingModels : [], requiresQuestionModel: ["A2", "B1"].includes(String(slide.course || "").toUpperCase()), suggestedMinutes: interactionMinutes(slide, 3) || 10 },
