...
import { resolveClassCohort, updateSession } from "../services/liveClassService.js";
...
export default function CanonicalAttendancePageV2() {
  ...
  const levelId = useMemo(() => resolveLevel(klass || {}), [klass]);
  ...
  async function changeAssignment(nextAssignmentId) {
    if (!selected || !nextAssignmentId) return;
    const entry = courseDictionary[levelId]?.[nextAssignmentId];
    const topic = getUnifiedTopicLabel(nextAssignmentId, entry?.en || entry?.de || selected.topic || "");
    setSessionBusy(true);
    try {
      await updateSession(selected.id, {
        assignmentIds: [nextAssignmentId],
        chapterIds: [nextAssignmentId],
        curriculumIds: [nextAssignmentId],
      ...
      success(`${nextAssignmentId} selected.`);
    } catch (cause) {
      error(cause?.message || "Could not update the curriculum assignment.");
    } finally {
    ...
    <section className="page-container" style={{ maxWidth: 1000 }}>
      ...
      {selected ? <>
        <article className="card">
          ...
          <label style={{ display: "grid", gap: 6, maxWidth: 650 }}>
            <strong>Curriculum assignment — all {dictionaryEntries.length} {levelId} items</strong>
            <select value={assignmentId} disabled={sessionBusy || sessionLocked || !dictionaryEntries.length} onChange={(event) => changeAssignment(event.target.value)}>
              <option value="">Select curriculum</option>
              {dictionaryEntries.map((entry) => <option key={entry.assignment_id} value={entry.assignment_id}>{entry.assignment_id} — {entry.en || entry.de}</option>)}
            </select>
          </label>
