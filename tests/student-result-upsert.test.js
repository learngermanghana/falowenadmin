...
test("browser sends result updates only through the authenticated integration hub", () => {
  const serviceSource = read("src/services/studentResultsService.js");
  const integrationSource = read("src/services/integrationEventService.js");
  ...
  assert.doesNotMatch(integrationSource, /SCORES_WEBHOOK_TOKEN/);
});

test("same-origin score compatibility API is admin-only and keeps webhook settings private", () => {
