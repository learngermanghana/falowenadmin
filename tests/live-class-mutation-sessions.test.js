...
test("superseded repair aliases do not consume curriculum positions before mutation enrichment", async () => {
  ...
  const berlinRows = [
    ...
    { id: "day3-alias-flag", classRecordId: berlin.id, className: berlin.name, startsAt: "2026-09-09T10:30:00Z", endsAt: "2026-09-09T11:30:00Z", assignmentIds: ["A1-1.1-PRACTICE"], superseded: true },
    { id: "day3", classRecordId: berlin.id, className: berlin.name, startsAt: "2026-09-09T11:00:00Z", endsAt: "2026-09-09T12:00:00Z", assignmentIds: ["A1-1.1-PRACTICE", "A1-1.2"], status: "scheduled" },
  ];
  const queryBerlin = async (field, identifier) => berlinRows.filter((row) => row[field] === identifier);
  const sessions = await loadMutationClassSessions(berlin.id, berlin, queryBerlin);
