...
test("student participation view resolves records saved by student code through the linked student profile", async () => {
  const studentDocs = [
    {
      id: "STACY123",
      data: {
        ...
        where(field, op, value) {
          ...
          return {
            limit(limit) {
              return {
                async get() {
                  return { docs: snapshots(matching.slice(0, limit)) };
                },
              };
            },
          };
  ...
  const rows = await participationApi.loadStudentParticipationForUser(db, {
    uid: "auth-uid-not-stored-on-participation",
    email: "stacy@example.com",
  });
