...
const { onDocumentWritten: onTrialStudentDocumentWritten } = require("firebase-functions/v2/firestore");
...
function buildMarkingPrompt(payload = {}) {
  return [
    ...
    "Use referenceEntry.writingParts, referenceEntry.aiGradedParts, and referenceEntry.partGrading as the source of truth for which part is Schreiben. A2/B1 normally use Teil 2 for writing, but A1 assignments may declare writing in another Teil. Never force a declared writing part to objective merely because it is Teil 3 or Teil 4. Use referenceAnswerParts/answer-key metadata for objective work. If any required objective answer key is missing, do not guess; mark needs_review.",
    "Every writing part declared by the manifest must be graded even when there is no fixed reference answer for that part. Never award 100 solely because objective questions are all correct when a writing section is present; include a writingScore and combine it with the objectiveScore for finalScore.",
    "When referenceEntry.questionAwareWritingTask is present, treat it as the canonical statement of the exact writing question, required communicative points, writing part(s), text type, and register. Grade against those requirements rather than a generic topic. For A1, simple correct task-appropriate German is enough: do not demand advanced connectors, complex sentences, or vocabulary beyond the task. Keep A1 corrections and suggested model phrases short and based on language the learner can reasonably use in that task, for example a simple health reason such as 'Ich bin krank.' when illness is the required reason.",
    "For writing, assess task completion, CEFR-appropriate grammar, word order, vocabulary, spelling, structure, clarity, and the required text form/register. Use the actual writing task and communicative points supplied in referenceEntry.partGrading/instructions and the submission metadata. Return writingStrengths as one or two short evidence-based strengths that quote or name exact details from the student's writing. Return taskCompletion as an object with completed, total, and missing. Return taskPointEvidence as one entry for every required communicative point, with label, status (met, missing, or review), a short exact evidence phrase from the declared writing part when met, and a brief reason. A question mark, greeting, connector, or closing counts only when it performs the communicative function requested by that point. Return corrections as one or two objects with from, to, reason, and the actual declared writing partId; use an empty array when there is no genuine correction. Content-point completion alone does not justify a perfect writing score: language accuracy, grammar, word order, spelling, capitalization, register, and email/letter form must affect the score. Score Schreiben as a 0-100 percentage and make the number agree with the evidence and feedback. For A2/B1, a coherent response that addresses the task, develops connected ideas, gives relevant examples, and shows several appropriate sentence structures should normally be at least 60 unless there are substantial language or task-completion problems. Scores below 60 are reserved for clearly insufficient writing and must be justified by concrete missing task points, serious recurring language problems, or major coherence/clarity failures that are explicitly reflected in corrections and feedback. Return nextStep as one specific task-relevant improvement or extension goal. Never invent a correction merely to fill a field, and avoid generic writing comments.",
...
const registrationLifecycleTriggers = createRegistrationLifecycleTriggers({
  admin,
  db,
  onDocumentCreated,
  onDocumentUpdated,
...
exports.sendTrialAccessWelcomeEmail = createTrialAccessWelcomeEmailTrigger({
  admin,
  db,
  onDocumentWritten: onTrialStudentDocumentWritten,
  runtimeConfig,
});
