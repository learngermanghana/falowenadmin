...
export function resolveSessionChapters`;
  ...
  // Always replace the whole generated completion block. Older checkouts can already
  // contain undoSessionCompletion while still using the legacy session.classId lookup.
...
function patchPage() {
  ...
  // Upgrade both a clean source file and pages generated by older versions of this patch.
  // Do not use the presence of the Undo action as a proxy for the canonical-class fix.
  if (!source.includes('await markSessionCompleted(session.id, adminId, canonicalClassId);')) {
  ...
  const curriculumSyncFeedback = `        const curriculumSyncError = String(next.curriculumSync?.error || "").trim();
        if (curriculumSyncError) {
          lastCurriculumSyncError = curriculumSyncError;
          setMessage(curriculumSyncError);
        } else if (lastCurriculumSyncError) {
          const recoveredCurriculumSyncError = lastCurriculumSyncError;
          lastCurriculumSyncError = "";
          setMessage((current) => current === recoveredCurriculumSyncError ? "" : current);
        }`;
...
${curriculumSyncFeedback}
    ...
    if (!source.includes('let lastCurriculumSyncError = "";')) {
      source = source.replace(
        "    let refreshInFlight = false;",
        '    let refreshInFlight = false;\n    let lastCurriculumSyncError = "";',
      );
    ...
    source = source.replace(
      /        (?:if \(next\.curriculumSync\?\.error\) setMessage\(next\.curriculumSync\.error\);|setMessage\(next\.curriculumSync\?\.error \|\| ""\);)/,
      curriculumSyncFeedback,
    );
  }
